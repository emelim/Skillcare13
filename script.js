...
function displayVideos(videos) {
    ...
    if (videos.length === 0) {
        ...
        videos.forEach(video => {
            ...
            overlay.classList.add("video-overlay"); // Add overlay
            ...
            // Adding the "See Video Description" button
            const descriptionButton = document.createElement("button");
            descriptionButton.classList.add("description-button");
            descriptionButton.textContent = "See video description";
            descriptionButton.onclick = () => showDescriptionModal(video.description);

            videoItem.appendChild(videoFrame);
            videoItem.appendChild(overlay); // Add overlay to video item
            videoItem.appendChild(videoTitle);
            videoItem.appendChild(videoDescription);
            videoItem.appendChild(videoTime);
            videoItem.appendChild(descriptionButton); // Add description button to video item
...
function showDescriptionModal(description) {
    ...
    window.scrollTo(0, 0); // Scroll to top of the page to show description
}

// Close the Video Description Modal
